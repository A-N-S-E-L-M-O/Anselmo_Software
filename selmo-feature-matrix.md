...
| Document RAG ("chat with files") | no (by design) | no | ext | OK (LocalDocs) | OK |
...
Three rows are Selmo-only across the whole set: **image generation shipped in the
box**, the **exhaustive document pipeline** (everyone else does retrieval-style
RAG, which is a different job), and the **energy monitor**. Those, plus the
family-grade portability, are the defensible ground — the multimodal features are
what make the package feel complete next to them.
