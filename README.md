...
- **Local image generation** via `stable-diffusion.cpp`, text-to-image and image-to-image, with automatic VRAM swap so the image model and the LLM can share a 12 GB GPU. Multiple models supported: Z-Image-Turbo (default), Z-Image-Base, FLUX.1-schnell, SD3.5-medium-turbo, Qwen-Image-2512.
...
| **8085** | LibreHardwareMonitor | Optional; real CPU/GPU power (served by the external LHM process). |
...
| **8888** | SearXNG | Podman container; the web bridge queries this before falling back to DDG. |
...
Performance is validated against the following setup, which supports **Qwen3 30B/35B-A3B** (MoE) at **>30 tokens/second**:
...
For optimal performance with large MoE models (e.g., Qwen3 30B/35B-A3B), the launcher supports `--n-cpu-moe` to offload experts to RAM, ensuring smooth inference even when VRAM is constrained.

### Dependencies

```bash
pip install flask faster-whisper pynvml trafilatura --break-system-packages
...
2. Launch the app. Two options with different startup behaviour:
   - **`Selmo.bat`**: opens an interactive model picker on every launch — choose the model, then press Enter.
   - **Tray app (`selmo_tray.py`)**: auto-loads the last-used model from `selmo-state.json` with no picker; if no model has been used yet, starts services only and lets you pick from the browser.
3. Open the client:
