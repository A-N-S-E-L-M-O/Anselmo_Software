...
*Updated session 27 · 2026-06-21 · v0.904*
...
| `selmo-settings.js` | tray-in-browser panel (model switch + service controls) | `openSettings` `closeSettings` `loadSettingsModels` `switchModel` `waitLLMReady` `unloadLLM` `reloadLLM` `exitSelmo` |
...
**Tray-in-browser (`selmo-settings.js`, started v0.903).** The tray's interactive parts are being moved into the browser so the UI is fully usable from any remote device. Opened by clicking the header `model:` label. Talks to the **control API on 8087** through the front door (`CTRL = /proxy/8087`). Shipped in v0.903: **LLM model switch** (lists `models\`, loads by name) plus **unload / reload / exit**. Still TODO: image-model pick and srv/chunking editing. The control API (`_CtrlHandler` in `selmo_tray.py`) now exposes: `GET /status`, `GET /models`, `POST /llm/unload`, `GET|POST /llm/reload`, `POST /llm/switch` (`{name}` -> reuses `_action_switch`), `POST /control/exit`. After a switch the panel polls `/props` until the model is up, then `location.reload()` so ctx/think/config refresh cleanly. NB: 8087 binds `0.0.0.0` with no auth (same trust model as the other LAN bridges) — switching/exiting is reachable by anyone on the LAN/VPN.
