...
*Updated session 27 · 2026-06-21 · v0.913*
...
- **`selmo-core.js` … `selmo-sessions.js` (8 modules)** contain **only function declarations**, grouped by subsystem. Load order among them does not matter — function declarations become global as each script runs.
...
NB: 8087 binds `0.0.0.0` with no auth (same trust model as the other LAN bridges) — switching/selecting/exiting is reachable by anyone on the LAN/VPN.

---

## Architecture at a glance

Selmo is one `llama.cpp` server, a small set of single-purpose Python bridges, and a single-page client (`chat.html` shell + 9 classic `.js` files — see "Client file map"). Everything runs locally; nothing leaves the machine except an explicit `/web` search.
