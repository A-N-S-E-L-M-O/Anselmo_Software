...
*Updated session 27 · 2026-06-21 · v0.901*
...
## Client file map (chat.html + selmo-app.js)

Since **v0.900** the client is two files, not one. `chat.html` is now just the **shell**: the `<head>`, the inline `<style>` block, the whole body markup (header, sidebar, chat pane, dashboard, modals), the CDN `<script>` tags, and one final `<script src="selmo-app.js?v=0.x"></script>`. **All application JavaScript lives in `selmo-app.js`**, extracted verbatim from the old inline script and loaded as a *classic* (non-module) script so every function stays global and the inline `onclick="fn()"` handlers keep working with zero rewiring. The front door (`selmo_https_proxy.py`) serves both straight from disk (`.js` mime added there, not blocked); bump the `?v=` query whenever `selmo-app.js` changes, because `.js` is cacheable while only `.html`/`.json` are sent `no-store`.

**Editing rules carry over.** Never use the Edit tool on `chat.html` (BUG-META-01) — Python via bash only. `selmo-app.js` is an ordinary file, but after any change run `node --check selmo-app.js` and verify with the Read tool; the mount can still serve stale/NUL views (BUG-META-02), so cross-check against `git show HEAD:` when anything looks off.

`selmo-app.js` is still one ~116 KB file. The next planned step is to subdivide it into per-subsystem `.js` files, each with its own `<script src>` (classic scripts, no bundler), landed as commits on the pure `.js` so `chat.html` is never touched again for it. Keep load order **`selmo-core.js` first**, then the rest, since the others read globals defined in core. This table is also the guide for **where to add new code**:

| Module | Responsibility | Key functions |
|---|---|---|
| `selmo-core.js` | globals, base URLs, config, power/cost gauges, server status, sessions, layout | `pt` `ap` `makeGauge` `setGauge` `setOdo` `poll` `paint` `savePrice` `updCost` `resetWh*` `loadProps` `checkServer` `addMsg` `newChat` `saveSession` `loadSession` `renderStored` `renderSessionList` `deleteSession` `mobToggle` `closeOverlays` `_onVVResize` `_micHttpsBanner` |
| `selmo-chat.js` | send loop, streaming, THINK control, token budget | `apiMessages` `maxTok` `sendMsg` `endTurn` `streamTokens` `makeThinkPanel` `stopMsg` `ensureLLM` `modelInfoLine` `traceLine` `announceModel` `setThinkEnabled` `syncThinkPrompt` `thinkKwargs` `configureThink` `applyThinkMode` `markThinking` `toggleThink` |
| `selmo-profiles.js` | Selmo / Mizan / Custom profiles + sampling | `activeSP` `syncSampling` `setProfile` `updateProfileUI` `onCustomParam` `openProfile` `closeProfile` |
| `selmo-docs.js` | file extraction, chunk pipeline, reply/chat export | `extractDocx/Odt/Pdf/Spreadsheet/Pptx/Odp` `loadFile` `clearFile` `detectHeadings` `splitChunks` `splitDoc` `processDoc` `processChunks` `askFileMode` `exportChat` `dlReplyMd` `dlReplyTsv` `mdTableToTSV` `addDownloadBar` `copyToClipboard` |
| `selmo-web.js` | WEB toggle + query rewrite | `toggleWeb` `webSearch` `rewriteQuery` `checkWebBridge` |
| `selmo-voice.js` | mic/Whisper, push-to-talk, VAD, Kokoro TTS | `toggleMic` `checkWhisperBridge` `pttStart` `pttStop` `f32ToWav` `vadSetState` `vadResume` `vadAfterSpeak` `vadOnSpeech` `toggleVad` `checkKokoroBridge` `toggleTts` `cleanForTts` `speakText` |
| `selmo-vision.js` | image input for vision / OCR | `imgToJpegDataUrl` `addImageFiles` `loadFileAsImage` `handlePaste` `imgThumbStrip` `pickImg` |
| `selmo-imagegen.js` | text-to-image / img2img | `toggleImgMenu` `genImage` `setImageMode` |

A few functions straddle two areas (`ensureLLM`, the reply-export helpers, the chunk pipeline); the table files each in its most-used home. **Future module:** `selmo-settings.js` — the tray-in-browser panel (LLM switch, image-model pick, srv/chunking edit, unload/reload/exit) — will be a new file talking to the extended control API on port 8087, so the tray's interactive parts become reachable from any remote browser without growing `chat.html`.

---

## Architecture at a glance

Selmo is one `llama.cpp` server, a small set of single-purpose Python bridges, and a single-page client (`chat.html` shell + `selmo-app.js`). Everything runs locally; nothing leaves the machine except an explicit `/web` search.
...
**Split `chat.html` into per-subsystem files** *(step 1 DONE in v0.900: the inline `<script>` was extracted verbatim into `selmo-app.js` — chat.html 158 KB → 42 KB. Remaining: subdivide `selmo-app.js` into the per-subsystem modules — see "Client file map" near the top for the target layout.)*
