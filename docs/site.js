...
var I18N = {
  en: {
    ...
    'cap.docs': 'Chunking & agent', 'cap.docs.d': 'Full-file analysis, auto-chunking, vision, OCR & agentic file access',
  ...
  es: {
    ...
    'cap.docs': 'Fragmentación y agente', 'cap.docs.d': 'Análisis de archivos completos, fragmentación automática, visión, OCR y acceso agente a archivos',
  ...
  zh: {
    ...
    'cap.docs': '分块与代理', 'cap.docs.d': '整文件分析、自动分块、视觉、OCR 与代理文件访问',
  ...
  ar: {
    ...
    'cap.docs': 'التقسيم والوكيل', 'cap.docs.d': 'تحليل الملف كاملًا، تقسيم تلقائي، رؤية وOCR ووصول وكيل للملفات',
  ...
  it: {
    'brand.mini': 'A.N.S.E.L.M.O · <b>IA locale</b>',
    'nav.back': '← Indietro',
    'hero.tag': "Il modo semplice per entrare nell’IA locale, senza stigma.",
    'hero.sub': 'Algorithm for Neural Synthesis with Emotional-Linguistic Memory Optimization. Selmo, per gli amici.',
    'entry.app.k': 'Il software', 'entry.app.t': 'Vuoi parlare con A.N.S.E.L.M.O dal tuo PC?',
    'entry.app.d': 'Per la lavatrice, non ci siamo ancora.',
    'entry.app.go': 'Entra →',
    'entry.book.k': 'Il libro', 'entry.book.t': 'The Washing Machine Dialogues',
    'entry.book.d': 'Il romanzo da cui è nato A.N.S.E.L.M.O.',
    'entry.book.go': 'Entra →',
    'sw.title': 'A.N.S.E.L.M.O', 'sw.sub': "Il modo semplice per entrare nell’IA locale, senza stigma.",
    'fw.title': 'A.N.S.E.L.M.O — interfaccia live', 'fw.hint': 'trascinami',
    'fw.ph': 'Carica lo screenshot dell’interfaccia in assets/anselmo-ui.png',
    'phone.ph': 'Carica lo screenshot del telefono in assets/anselmo-phone.jpg',
    'caps.h': 'Funzionalità principali',
    'cap.energy': 'Monitoraggio energetico', 'cap.energy.d': 'Watt in tempo reale + Wh per sessione',
    'cap.think': 'Ragionamento / THINK', 'cap.think.d': 'Pannello dedicato ai modelli di ragionamento',
    'cap.mobile': 'Accesso mobile', 'cap.mobile.d': 'Accesso dal telefono via HTTPS sulla rete locale',
    'cap.docs': 'Chunking & agente', 'cap.docs.d': 'Analisi file completi, chunking automatico, visione, OCR e accesso agente ai file',
    'cap.voice': 'Loop vocale', 'cap.voice.d': 'Whisper STT + Kokoro TTS, mani libere',
    'cap.web': 'Ricerca web', 'cap.web.d': 'Opzionale, disattivata di default, privacy prima di tutto',
    'cap.img': 'Da immagine a immagine', 'cap.img.d': 'Genera da testo o da immagine esistente',
    'cap.chat': 'Chat', 'cap.chat.d': 'Conversazione locale, zero cloud',
    'faq.h': 'Sul software',
    'cta.github': 'Scarica da GitHub', 'cta.github.s': 'Download · sorgente disponibile',
    'cta.quickstart': 'Guida rapida — operativo in 3 passi →',
    'cta.readbook': 'Leggi il libro', 'cta.readbook.s': 'The Washing Machine Dialogues',
    'cta.note': 'Solo Windows per ora. Decomprimi e clicca — nessuna installazione, nessun account.',
    'book.title': 'The Washing Machine Dialogues', 'book.sub': 'Dialoghi con la lavatrice — il romanzo dietro A.N.S.E.L.M.O.',
    'book.pitch': "Il &laquo;padre&raquo; di Selmo viene chiamato a rispondere a una domanda che nessun algoritmo può risolvere da solo: qual è la cosa giusta da fare. Dal quartiere Ortica ai corridoi del potere finanziario, dai teatri sotterranei ai campi di assistenza umanitaria, prende forma Arx &mdash; un progetto che spera di intervenire senza dominare. Ma ogni scelta ha conseguenze, e non tutte sono reversibili. <i>The Washing Machine Dialogues</i> è un romanzo solarpunk lucido e profondamente umano sul potere quando rinuncia all’eroismo, e sulla tecnologia quando è costretta a fare i conti con la responsabilità.",
    'book.avail': 'Il libro è disponibile qui:',
    'cta.amazon': 'Amazon', 'cta.amazon.s': 'Cartaceo · eBook',
    'cta.kobo': 'Kobo', 'cta.kobo.s': 'eBook',
    'foot.quick': 'Guida rapida', 'foot.terms': 'Termini di utilizzo', 'foot.notice': 'Avviso e licenza',
    'foot.cw': '© 2026 Fabio Garzetti — A.N.S.E.L.M.O è a sorgente disponibile (Apache 2.0 + Commons Clause).'
  }
};
...
var FAQ = {
  en: [
    ...
    { q:'Does Selmo have an agent mode?',
      body:"<p>Yes. Agent mode turns Selmo into a multi-step autonomous loop: instead of answering once and stopping, Selmo can plan a task, call tools to gather information, act on what it finds, and keep going until the job is done — or until it decides to ask you.</p><p>The tools available to the agent give it controlled access to your local file system: it can list directories, read files, and search their contents with grep-style queries. Access is limited to the folders you explicitly configure — Selmo never touches anything outside that boundary.</p><p>For desktop applications, Selmo can interact with Firefox, Thunderbird, and LibreOffice if they are declared in the configuration. This lets the agent open a document, read a web page, or work through an email thread as part of a longer task.</p><p>In the current version, file access is read-only: the agent can read and search but not write or delete. Future versions will extend this. Agent mode activates only on models that declare the capability in <code>selmo-models.ini</code> — the button is greyed and labelled accordingly when the loaded model doesn't support it.</p>" },
  ...
  es: [
    ...
    { q:'¿Tiene Selmo un modo agente?',
      body:"<p>Sí. El modo agente convierte a Selmo en un bucle autónomo de varios pasos: en lugar de responder una vez y detenerse, Selmo puede planificar una tarea, llamar herramientas para recopilar información, actuar según lo que encuentre y continuar hasta que la tarea esté terminada — o hasta que decida preguntarte.</p><p>Las herramientas disponibles para el agente le dan acceso controlado a tu sistema de archivos local: puede listar directorios, leer archivos y buscar su contenido con consultas tipo grep. El acceso se limita a las carpetas que configures explícitamente — Selmo nunca toca nada fuera de ese límite.</p><p>Para aplicaciones de escritorio, Selmo puede interactuar con Firefox, Thunderbird y LibreOffice si están declarados en la configuración. Esto permite al agente abrir un documento, leer una página web o trabajar con un hilo de correo como parte de una tarea más larga.</p><p>En la versión actual, el acceso a archivos es de solo lectura: el agente puede leer y buscar pero no escribir ni eliminar. Las versiones futuras ampliarán esto. El modo agente se activa solo en los modelos que declaran la capacidad en <code>selmo-models.ini</code> — el botón aparece en gris con la etiqueta correspondiente cuando el modelo cargado no lo admite.</p>" },
  ...
  zh: [
    ...
    { q:'Selmo 有代理模式吗？',
      body:"<p>有。代理模式将 Selmo 变成一个多步骤自主循环：它不再只回答一次就停下，而是能够规划任务、调用工具收集信息、根据发现采取行动，并持续推进直到任务完成——或者决定向你提问。</p><p>代理可用的工具让它对本地文件系统有受控的访问权限：它可以列出目录、读取文件，并用类 grep 的查询搜索文件内容。访问范围限定在你明确配置的文件夹内——Selmo 绝不触碰这个边界之外的任何内容。</p><p>对于桌面应用程序，如果在配置中声明，Selmo 可以与 Firefox、Thunderbird 和 LibreOffice 交互。这让代理能作为更长任务的一部分打开文档、阅读网页或处理邮件线程。</p><p>当前版本中，文件访问是只读的：代理可以读取和搜索，但不能写入或删除。未来版本将扩展这一功能。代理模式仅在 <code>selmo-models.ini</code> 中声明了该能力的模型上激活——当加载的模型不支持时，按钮会变灰并相应标注。</p>" },
  ...
  it: [
    { q:'Che cos\'è A.N.S.E.L.M.O?',
      body:"<p>Prima di tutto, cosa <b>non</b> è: A.N.S.E.L.M.O non è un'IA, e non allena nessun modello proprio. È un'<b>interfaccia grafica</b> — una pagina pulita — che pilota motori e modelli open source di terze parti sul <b>tuo computer</b>. L'intelligenza vive nei modelli che carichi (llama.cpp per il linguaggio, Whisper per il parlato, Kokoro per la voce, stable-diffusion.cpp per le immagini); Selmo è la cabina di pilotaggio che li rende usabili insieme.</p><p>Attraverso quell'unica interfaccia hai chat, visione e OCR, analisi di documenti interi, ricerca web opzionale, voce e generazione di immagini — tutto da una sola macchina.</p><p>Il nome è un acronimo: <i>Algorithm for Neural Synthesis with Emotional-Linguistic Memory Optimization</i>. Selmo, per gli amici. Viene dal romanzo <i>The Washing Machine Dialogues</i>, dove Selmo è un personaggio prima di essere un prodotto.</p>" },
    { q:'Perché A.N.S.E.L.M.O?',
      body:"<p>Perché le tue conversazioni non dovrebbero dover lasciare la tua scrivania. Niente cloud, niente account, niente telemetria — e poiché il sorgente è disponibile, puoi verificare che sia vero.</p><p>Ti mostra anche una stima dell'energia consumata: watt ed energia per token. «Etico» diventa un numero sullo schermo invece di uno slogan. Fatto in Europa, privato per design.</p>" },
    { q:'Per chi è?',
      body:"<p>Per chiunque voglia una vera IA sul proprio hardware senza consegnare nulla a un cloud: persone attente alla privacy, famiglie, studenti, professionisti che lavorano con documenti sensibili.</p><p>È decomprimi e clicca — non devi essere uno sviluppatore. Gli sviluppatori sono i benvenuti, ma non sono quelli per cui l'abbiamo costruito prima.</p>" },
    { q:'Cosa fa?',
      body:"<p>Esegue qualsiasi modello open source (licenza permissiva) sopra <code>llama.cpp</code>, dietro una pagina pulita. Ti offre:</p><p>• Chat con tre profili selezionabili (Selmo, Mizan, Custom).<br>• Lettura e analisi di documenti interi (docx, pdf, xlsx, pptx, odt…) con copertura esaustiva, non RAG a frammenti.<br>• Visione e OCR su immagini e PDF.<br>• Ricerca web opzionale, disattivata di default.<br>• Voce in entrata e uscita (Whisper + Kokoro), mani libere.<br>• Generazione di immagini locale.<br>• Il tuo telefono che parla con il modello del tuo PC sulla rete di casa.</p>" },
    { q:'Quanta energia consuma — e posso vederlo?',
      body:"<p>Sì — ed è la parte che nessun'altra app di IA locale ti dà. A.N.S.E.L.M.O mostra una <b>stima</b> della potenza che la tua macchina consuma mentre lavora (CPU più GPU più il resto), i wattora spesi in questa sessione e in totale, la velocità in token al secondo e il costo corrente nella tua valuta, dal prezzo che paghi per kWh.</p><p>È una stima, non un misuratore certificato: il consumo della GPU viene letto dalla scheda, CPU e resto vengono approssimati dal carico. È abbastanza precisa per confrontare un modello leggero con uno pesante sullo stesso hardware. Per il dato esatto, un misuratore di potenza a parete o una presa smart con misurazione energetica legge quello che il PC preleva davvero dalla presa, e puoi calibrare Selmo su quello.</p>" },
    { q:'Aspetta — dal mio telefono?',
      body:"<p>Sì. Selmo gira sul tuo PC, ma serve la sua interfaccia sulla tua rete di casa, quindi qualsiasi dispositivo sulla stessa Wi-Fi — il tuo telefono, un tablet — può aprirla in un browser. Sul telefono vai su <code>https://IP-DEL-TUO-PC:8443/chat.html</code> (l'indirizzo del PC è mostrato nell'icona del tray), accetta l'avviso del certificato una volta, e stai parlando con il modello del tuo PC. Quell'avviso una tantum è normale — è ciò che permette al telefono di usare il microfono su una connessione sicura.</p><p>Nulla tocca un cloud; resta sulla tua rete. Per raggiungere Selmo quando sei fuori casa, non esporre il PC alla rete aperta — usa invece una VPN verso il tuo router (WireGuard o OpenVPN sul router, o una VPN mesh come Tailscale), poi usa lo stesso indirizzo del PC come se fossi a casa.</p>" },
    { q:'Ho provato Selmo e parla di «chunking» — cosa significa?',
      body:"<p>Un modello può tenere in memoria di lavoro solo una certa quantità di testo alla volta (la sua finestra di contesto). Quando gli dai un documento lungo che non ci entra, il <b>chunking</b> è come divide il file in pezzi che ci entrano, li elabora uno per uno, e poi ricuce i risultati con un riepilogo finale.</p><p>Il punto è la copertura: Selmo percorre il documento <b>intero</b>, passaggio per passaggio, invece di prendere qualche frammento che ha ipotizzato rilevante (la scorciatoia che usano la maggior parte degli strumenti). Così una traduzione o un'analisi di un file lungo è esaustiva, senza nulla saltato in silenzio. Quando Selmo dice che sta facendo chunking, ti sta dicendo che un file grande viene gestito per intero invece di essere troncato.</p>" },
    { q:'Selmo ha una modalità agente?',
      body:"<p>Sì. La modalità agente trasforma Selmo in un ciclo autonomo a più passi: invece di rispondere una volta e fermarsi, Selmo può pianificare un compito, chiamare strumenti per raccogliere informazioni, agire in base a ciò che trova e andare avanti fino al termine del lavoro — o finché non decide di chiederti qualcosa.</p><p>Gli strumenti disponibili per l'agente gli danno accesso controllato al file system locale: può elencare directory, leggere file e cercarne il contenuto con query in stile grep. L'accesso è limitato alle cartelle che configuri esplicitamente — Selmo non tocca mai nulla al di fuori di quel confine.</p><p>Per le applicazioni desktop, Selmo può interagire con Firefox, Thunderbird e LibreOffice se dichiarati nella configurazione. Questo permette all'agente di aprire un documento, leggere una pagina web o lavorare su un thread di posta come parte di un compito più lungo.</p><p>Nella versione attuale, l'accesso ai file è in sola lettura: l'agente può leggere e cercare ma non scrivere né eliminare. Le versioni future estenderanno questo. La modalità agente si attiva solo sui modelli che dichiarano la capacità in <code>selmo-models.ini</code> — il pulsante appare grigio con l'etichetta corrispondente quando il modello caricato non la supporta.</p>" },
    { q:'Ho bisogno di un PC mostro? Cosa devo aspettarmi?',
      body:"<p>No. A.N.S.E.L.M.O esegue qualsiasi modello che entri nella tua macchina — più grande e veloce è la GPU, più grande il modello e più token al secondo ottieni. Su una GPU da gaming di fascia media con 12 GB di VRAM esegue comodamente modelli da 24 miliardi di parametri; su hardware più leggero esegue modelli più piccoli, più lentamente.</p><p><b>Un trucco che strizza l'hardware modesto: i modelli Mixture-of-Experts (MoE).</b> Un MoE è grande in totale ma attiva solo pochi miliardi di parametri per token, così un MoE da 30B con circa 3B attivi va veloce anche quando l'intero non entra in VRAM — Selmo tiene il nucleo sulla GPU e spinge gli esperti inattivi nella RAM di sistema. Su una GPU piccola è di solito la scelta migliore, dandoti gran parte della qualità di un modello grande senza la VRAM che richiederebbe un modello denso delle stesse dimensioni. Li raccomandiamo.</p><p>Non c'è un minimo «PC mostro»: scambi dimensione e velocità del modello con quello che hai. E poiché tutto gira in locale, l'elettricità è l'unico costo corrente — ed è esattamente per questo che A.N.S.E.L.M.O tiene a schermo una stima dei watt e dell'energia per token.</p>" },
    { q:'Cosa NON fa?',
      body:"<p>Non telefona a casa. Niente telemetria, niente analytics, nessun account, nessun canale di rete nascosto — nulla raggiunge l'autore, nemmeno il fatto che l'hai installato.</p><p>Non è un servizio cloud: non c'è un server che appartiene a qualcuno. Non è una garanzia di correttezza — l'output dell'IA può essere sbagliato, quindi verifica ciò che conta.</p>" },
    { q:'È pericoloso?',
      body:"<p>Non per la tua privacy: nulla lascia la tua macchina, nessun account né telemetria, e il sorgente è disponibile per verificarlo.</p><p>Due avvertenze oneste. Può servire la sua interfaccia ad altri dispositivi sulla tua rete di casa — così funziona l'accesso dal telefono — quindi usalo su reti di cui ti fidi. E un modello locale non ha filtro contenuti cloud, quindi ciò che genera è responsabilità tua; con bambini intorno, la configurazione sicura è non installare il modello immagini e supervisionare. L'IA può anche sbagliare con sicurezza, quindi controlla ciò che conta. Al di là di questo, è un programma normale che gira sul tuo computer.</p>" },
    { q:'Posso usarlo per i miei progetti?',
      body:"<p>Sì. Il sorgente è disponibile sotto <b>Apache 2.0 con la Commons Clause</b>: usalo, studialo, modificalo, redistribuiscilo. L'unico limite — non puoi <b>venderlo</b>. È richiesta l'attribuzione al progetto A.N.S.E.L.M.O e al libro.</p><p>Se ci costruisci qualcosa sopra, deve portare un nome diverso: i nomi A.N.S.E.L.M.O / Selmo / Mizan restano al progetto. Tutti i dettagli nei Termini di utilizzo e nel file NOTICE.</p>" },
    { q:'Sono un insegnante, vivo in un posto remoto e la mia lingua è parlata da 3.000 persone. Posso avere Selmo nella mia lingua?',
      body:"<p>Prego, fallo — è una delle ragioni per cui Selmo esiste. Un'azienda cloud non localizzerà mai il suo prodotto per una lingua parlata da tremila persone; non c'è mercato. Uno strumento locale e aperto non ha questa scusa: l'interfaccia è solo testo in un file, e un parlante come te può tradurlo.</p><p>Ecco come. Le stringhe dell'interfaccia vivono in un file di lingua, <code>selmo-i18n.js</code> — una lista di chiavi brevi, ognuna con il suo testo in inglese. Copi il blocco inglese, lasci le chiavi esattamente come sono, e traduci il testo nella tua lingua. Selmo poi offre la tua lingua nel menu del globo, così l'interfaccia diventa tua.</p><p>Un limite onesto: la conversazione dipende dal modello che carichi, non da Selmo. Selmo è l'interfaccia; la capacità linguistica vive nel modello. Quindi vorrai un modello che conosca davvero la tua lingua — e per una lingua parlata da poche migliaia di persone, uno forte potrebbe non esistere ancora. L'interfaccia, però, è interamente nelle tue mani.</p><p>E una volta che Selmo e un modello sono sul PC, non ha bisogno di internet per niente — gira completamente offline, sul tuo hardware, che è esattamente lo spirito del progetto. Dove la connessione è scarsa o assente, questa è la differenza tra poter usare questi modelli o no.</p>" },
    { q:'Bah, sembra un trucco di marketing per vendere il libro.',
      body:"<p>Hai assolutamente ragione. L'app e il libro si promuovono a vicenda, e non lo negherò.</p><p>Ma A.N.S.E.L.M.O è gratuito e a sorgente disponibile, e lo uso davvero, ogni giorno, su una GPU con 12 GB di VRAM.</p>" },
    { q:'Risorse, crediti e ringraziamenti',
      body:"<p>A.N.S.E.L.M.O si regge sul lavoro open source — è un'interfaccia sottile, l'ingegneria difficile è loro. Grazie a tutti coloro che costruiscono e mantengono:</p><p><b>Motori e runtime:</b> llama.cpp (MIT), stable-diffusion.cpp (MIT), onnxruntime / onnxruntime-web (MIT).</p><p><b>Parlato e voce:</b> faster-whisper + CTranslate2 + Whisper (MIT), Kokoro / kokoro-onnx (Apache 2.0), @ricky0123/vad-web (ISC), Silero VAD (MIT), soundfile (BSD-3), langdetect (Apache 2.0).</p><p><b>Web, documenti e server:</b> Flask (BSD-3), requests (Apache 2.0), trafilatura (Apache 2.0), SearXNG (AGPL-3.0, processo separato), Podman (Apache 2.0), JSZip (MIT/GPL), SheetJS (Apache 2.0), PDF.js (Apache 2.0), marked (MIT).</p><p><b>Hardware e potenza:</b> pynvml (BSD-3), psutil (BSD-3), LibreHardwareMonitor (opzionale).</p><p><b>Modelli e pesi:</b> gli LLM open-weight che carichi — EuroLLM, Mistral, Gemma e altri, ognuno sotto la propria licenza — più Z-Image-Turbo (Apache 2.0), il text encoder Qwen3-4B (Apache 2.0) e il FLUX VAE (Apache 2.0).</p><p><b>Carattere tipografico:</b> Share Tech Mono (SIL OFL).</p><p>Licenze complete e attribuzioni nel file NOTICE del progetto e nella sezione Crediti della documentazione.</p>" }
  ],
  ar: [
    ...
    { q:'هل يملك Selmo وضع الوكيل؟',
      body:"<p>نعم. يحوّل وضع الوكيل Selmo إلى حلقة مستقلة متعددة الخطوات: بدلًا من الإجابة مرةً واحدة ثم التوقف، يستطيع Selmo تخطيط مهمة، واستدعاء أدوات لجمع المعلومات، والتصرف وفق ما يجده، والمضيّ قدمًا حتى ينجز العمل — أو يقرّر أن يسألك.</p><p>الأدوات المتاحة للوكيل تمنحه وصولًا محكومًا إلى نظام الملفات المحلي: يستطيع سرد الأدلة، وقراءة الملفات، والبحث في محتواها باستفسارات من نوع grep. يقتصر الوصول على المجلدات التي تُحدّدها صراحةً — لا يلمس Selmo شيئًا خارج تلك الحدود.</p><p>بالنسبة لتطبيقات سطح المكتب، يستطيع Selmo التفاعل مع Firefox وThunderbird وLibreOffice إذا كانت مُعلَنةً في الإعداد. يتيح ذلك للوكيل فتح مستند، أو قراءة صفحة ويب، أو العمل خلال سلسلة بريد إلكتروني كجزء من مهمة أطول.</p><p>في الإصدار الحالي، الوصول إلى الملفات للقراءة فقط: يستطيع الوكيل القراءة والبحث لكن لا الكتابة أو الحذف. ستوسّع الإصدارات المقبلة هذا. يتفعّل وضع الوكيل فحسب على النماذج التي تُعلن القدرة في <code>selmo-models.ini</code> — يظهر الزر رماديًا مع تسمية مناسبة حين لا يدعمه النموذج المحمَّل.</p>" },
